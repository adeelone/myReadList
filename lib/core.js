...
export const MAX_LIBRARY = 5000;
export const MAX_CSV_CHARACTERS = 5_000_000;

export function parseCsv(text) {
  const source = String(text || "");
  if (source.length > MAX_CSV_CHARACTERS) throw new Error("Snapshot is larger than the 5 MB safety limit.");
  const records = [];
  let cell = "";
  let row = [];
  let quoted = false;
  ...
  for (let index = 0; index < source.length; index += 1) {
    ...
    if (character === '"') {
      ...
      if (row.some((value) => value !== "")) {
        records.push(row);
        if (records.length > MAX_LIBRARY + 1) throw new Error(`Snapshot exceeds the ${MAX_LIBRARY.toLocaleString()} novel safety limit.`);
      }
      row = [];
      cell = "";
    } else {
...
export function normalizeRow(row, fallbackOrder = 0) {
  ...
  return {
    ...
    source: String(row.Source || sourceName(bookUrl)).trim(),
    bookUrl,
...
export function normalizeState(value) {
  ...
  return {
    ...
    library: Array.isArray(source.library) ? source.library.slice(0, MAX_LIBRARY).map(normalizeBook).filter((book) => book.title) : [],
...
export function mergeSnapshot(currentState, rows, options = {}) {
  if (!Array.isArray(rows) || rows.length > MAX_LIBRARY) throw new Error(`Snapshot exceeds the ${MAX_LIBRARY.toLocaleString()} novel safety limit.`);
  const current = normalizeState(currentState);
  const importedAt = validDate(options.importedAt) || new Date().toISOString();
  const fileName = cleanFileName(options.fileName) || "Reading snapshot";
  ...
  const next = {
    ...
    imports: [{ id: importedAt, importedAt, fileName, sources: [...new Set(library.map((book) => book.source).filter(Boolean))].join(" + "), novelCount: library.length, changes: changes.length }, ...current.imports].slice(0, MAX_IMPORTS)
  };
...
export function createPublicSnapshot(value, generatedAt = new Date().toISOString()) {
  ...
  return {
    ...
    imports: state.imports.map(({ fileName: _fileName, ...item }) => ({ ...item, label: item.sources ? `${item.sources} snapshot` : "Reading snapshot" }))
  };
}
...
function normalizeBook(value) {
  ...
  return {
    ...
    source: String(value?.source || sourceName(bookUrl)).trim().slice(0, 100),
    bookUrl,
...
function normalizeHistoryEvent(value) {
  return {
    ...
    title: String(value?.title || "").trim().slice(0, 500),
    source: String(value?.source || sourceName(value?.bookUrl)).trim().slice(0, 100),
...
function normalizeImport(value) {
  return {
    ...
    sources: String(value?.sources || "").trim().slice(0, 200),
...
function sourceName(value) {
  try {
    ...
    if (host === "novelfire.net") return "NovelFire";
    if (host === "novelphoenix.com") return "NovelPhoenix.com";
    return host;
  } catch (_) { return ""; }
}
